...
void menuInicial() {
	...
	do {
		...
		printf("\n\t\t\tMENU INICIAL\n");
		printf("\t\t\t============\n");
		printf("\t\t9959-21-4557|| Jose Rodrigo Perez Cojon\n");
	...
	do {
		...
		if (!existeUsuario(nombreUsuario, &usuario)) {
			...
			if (insertarUsuario(usuario)) {
				printf("\n\tEl Entrenador fue registrado satisfactoriamente!\n");

			} else {
				printf("\n\tOcurrio un error al registrar el Entrenador\n");
				printf("\nInténtelo mas tarde\n");
			}
		} else {
			printf("\n\tEl Entrenador \"%s\" ya ha sido registrado previamente\n", usuario.nombre);
			printf("\tNo puede registrar dos entrenadores con el mismo nombre.\n");
		}

		printf("\n\tDesea seguir registrando Entrenadores? [S/N]: ");
...
void menuSistema() {
	...
	printf("\t\t    Sistema ¨de gestion del PoliDeportivo \n");
	printf("\t\t    9959-21-4557 | Jose Rodrigo Perez Cojon\n");
	printf("     ======================================================================\n");

	printf("\n\t\t[1]. Ver Equipos Registrados \n");
    printf("\t\t[2]. Registrar Equipo Nuevo\n");
	printf("\t\t[3]. Elminar equipo\n");
...
void menuRegistrarEquipo() {
	...
	do {
		...
		if (!existeUsuario(nombreUsuario, &usuario)) {
			...
			/* Se inserta el usuario en el archivo de usuarios */
			if (insertarUsuario(usuario)) {
				printf("\n\tEl equipo fue registrado satisfactoriamente!\n");

			} else {
				printf("\n\tOcurrio un error al registrar el equipo\n");
				printf("\nInténtelo mas tarde\n");
			}
		} else {
			printf("\n\tEl equipo \"%s\" ya ha sido registrado previamente\n", usuario.nombre);
			printf("\tNo puede registrar dos equipos con el mismo nombre.\n");
		}

		printf("\n\tDesea seguir registrando Equipos? [S/N]: ");
		leerLinea(respuesta, MAX);
